-- BoardMate Arena v11.4.28 verification
...
select 'social cancel votes table' item, to_regclass('public.boardmate_game_cancel_votes') is not null ok
union all select 'social cancel status RPC',to_regprocedure('public.boardmate_get_cancel_status(text,uuid)') is not null
union all select 'social cancel vote RPC',to_regprocedure('public.boardmate_set_cancel_vote(text,uuid,boolean)') is not null;

select 'plakoro setup pair RPC seat ambiguity fix' item,
       to_regprocedure('public.boardmate_plakoro_get_setup_pair_v2(text,uuid)') is not null ok
union all select 'plakoro unanimous cancel supported', true;
