...
async function renderCreateRoom(){
  ...
  document.querySelector('#createRoomBtn').onclick=async()=>{const title=document.querySelector('#roomTitle').value.trim(),st=document.querySelector('#roomStatus');try{const id=await callRpc('create_boardmate_room_v8',{p_token:memberToken(),p_title:title,p_game:selected});location.hash=`#/room/${id}`;}catch(e){st.textContent=e.message;}};
}
...
async function renderPensterdam(){
  ...
  const placementAt=boardAnchor=>{const ar=Math.floor(boardAnchor/7),ac=boardAnchor%7,cur=currentPiece(),coords=cur.points.map(([r,c])=>[ar+(r-cur.anchor[0]),ac+(c-cur.anchor[1])]),cells=coords.map(([r,c])=>r*7+c),valid=coords.every(([r,c])=>r>=0&&r<10&&c>=0&&c<7)&&cells.every(i=>PENTORINI_VALID.has(i)&&board[i]==null);return valid?{anchor:boardAnchor,coords,cells}:null;};
  const legalPlacements=()=>[...PENTORINI_VALID].map(placementAt).filter(Boolean);
  const previewLayerHtml=()=>{const placements=legalPlacements();return `<div class="pento-ghost-layer" aria-hidden="true">${placements.map((pl,pi)=>{const set=new Set(pl.coords.map(([r,c])=>`${r},${c}`));return pl.coords.map(([r,c])=>`<span class="pento-ghost-cell joined-piece ${HELPER_NAMES.includes(selected)?'piece-helper':`piece-${selected}`} ${joinedCellClasses((rr,cc)=>set.has(`${rr},${cc}`),r,c)}" style="--ghost-row:${r};--ghost-col:${c}"></span>`).join('')}).join('')}</div>`;};
  const renderBoard=()=>{const el=document.querySelector('#pboard');el.innerHTML=Array.from({length:70},(_,i)=>{if(!PENTORINI_VALID.has(i))return `<div class="p-cell invalid-cell"></div>`;const v=board[i],label=i<63?PENTORINI_LABELS[i]:'';return `<button class="p-cell ${pentoriniCellMeta(i,weekdayMode)} ${v==='#'?'today-hole':''} ${v&&v!=='#'?'occupied':''}" data-cell="${i}"><span class="calendar-label">${label}</span>${v&&v!=='#'?`<span class="piece-fill joined-piece ${HELPER_NAMES.includes(v)?'piece-helper':`piece-${v}`} ${boardPieceClasses(i,v)}"></span>`:''}</button>`;}).join('')+previewLayerHtml();el.querySelectorAll('[data-cell]').forEach(c=>c.onclick=()=>{const i=Number(c.dataset.cell),v=board[i];if(v==='#')return;if(v){removePiece(v);selected=v;rotation=0;flipped=false;renderAll();}else tryPlace(i);});document.querySelector('#helperUsed').textContent=`${helpUsed()} / 5`;};
  ...
  document.querySelectorAll('[data-weekday-mode]').forEach(b=>b.onclick=()=>changeWeekdayMode(b.dataset.weekdayMode));document.querySelector('#rotatePiece').onclick=()=>{if(HELPER_NAMES.includes(selected))return;rotation=(rotation+1)%4;renderAll();};document.querySelector('#flipPiece').onclick=()=>{if(HELPER_NAMES.includes(selected))return;flipped=!flipped;renderAll();};document.querySelector('#resetPento').onclick=()=>{reset();toast('도전을 초기화했습니다.');};renderAll();
}
