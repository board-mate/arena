...
function gameInfo(game){
  const map={
    ...
    fantasyrealms:{name:'판타지 왕국',icon:'🏰',min:2,max:6,page:'online-fantasy-realms.html'},
    fantasyrealmsgreek:{name:'판타지 왕국: 그리스의 전설들',icon:'🏛️',min:2,max:6,page:'online-fantasy-realms-greek.html'},
    ...
    projectl:{name:'Project L',icon:'🧩',min:2,max:4,page:'online-projectl.html',tier:'alpha'},
...
async function renderHome(){
  ...
  <section class="home-mode-grid"><button class="mode-card" data-go="solo"><span>🧠</span><b>1인플 · AI/솔로</b><small>마스크맨 / 어콰이어 / 에친스톤 / 캘리코 / 캐스캐디아 / 더 게임 / 커피 로스터</small></button><button class="mode-card" data-go="multi"><span>🌐</span><b>다인플 · 온라인 방</b><small>자동 저장 · 재접속 · 게임별 티어</small></button></section>
...
function renderSolo(){
  ...
  <section class="library-grid">
    ...
    <article class="library-card etchinstone"><div class="library-icon">🐉</div><h2>에친스톤의 용들</h2><p>4개 지역과 최후의 여정·용을 돌파하는 1인 손패 관리 모험입니다.</p><span class="save-badge">💾 로컬 자동 저장</span><a class="primary link-btn" href="./solo-etchinstone.html">솔로 플레이</a></article>
    ...
    <article class="library-card conservas"><div class="library-icon">🐟</div><h2>Conservas</h2><p>바다의 지속 가능성과 수익을 함께 관리하는 1인 어업·통조림 게임. 1월 시나리오부터 구현했습니다.</p><span class="save-badge">💾 로컬 자동 저장 · ALPHA</span><a class="primary link-btn" href="./solo-conservas.html">솔로 플레이</a></article>
  </section>`);
  document.querySelector('#backHome').onclick=()=>location.hash='#/';
}
...
async function renderCreateRoom(){
  ...
  const games=['maskmen','acquire','calico','cascadia','plakoro','powergrid','quacks','mandom','planetx','samurai','eldorado','panam','projectl','airlandsea','thegame','kraken','fantasyrealms','fantasyrealmsgreek','avalon','secrethitler','onenightwerewolf'];let selected='maskmen';
  ...
  document.querySelector('#createRoomBtn').onclick=async()=>{
    ...
    const v21Games=['quacks','mandom','samurai','eldorado','panam','projectl','airlandsea','planetx'];
    try{
      let id;
      try{
        id=await callRpc('create_boardmate_room_v10',{p_token:memberToken(),p_title:title,p_game:selected});
      }catch(v10e){
        const msg=String(v10e?.message||v10e);
        ...
        if(v21Games.includes(selected)&&(v10missing||catalogRejected)){
          ...
          if(selected==='projectl') throw new Error('Project L용 Supabase 업데이트가 필요합니다. SUPABASE_PROJECT_L_V1.sql을 SQL Editor에서 실행하세요.');
...
async function renderRoom(roomId){
  ...
  const draw=async()=>{data=await load();const {room,members}=data,isHost=room.host_id===me.user_id,gi=gameInfo(room.game),min=(room.game==='powergrid'?Math.max(3,Number(room.min_players||gi.min)):Number(room.min_players||gi.min));void processSingleRoomAlarm(room,me,{gameName:gi.name,gameHref:gameEntryHref(room)});
    ...
    if(!cancelled&&room.status==='finished'&&['maskmen','acquire','calico','cascadia','pocketnova','thegame','kraken','fantasyrealms','fantasyrealmsgreek','powergrid','avalon','secrethitler','onenightwerewolf','plakoro','quacks','mandom','samurai','eldorado','panam','projectl','airlandsea','planetx'].includes(room.game)){
    ...
    <div class="lobby-actions">${room.status==='open'&&isHost?`<button class="primary" id="startRoom" ${members.length<min?'disabled':''}>${members.length<min?`${min}명부터 시작 가능`:'게임 시작'}</button>`:''}${room.status==='open'?'<button class="danger" id="leaveRoom">방 나가기</button>':''}${room.status==='playing'?`<a class="primary link-btn" href="./${gameInfo(room.game).page||(`online-${room.game}.html`)}?room=${encodeURIComponent(room.id)}">이어하기 / 게임 입장</a><button class="ghost" id="disconnectRoom">나가기 (게임 저장 유지)</button>`:''}</div>
    <p class="pento-tip">게임 상태는 행동할 때마다 Supabase에 자동 저장됩니다. 브라우저를 닫거나 다른 페이지로 나가도 참가 자리는 남으며, 다인플의 ‘진행 중인 게임’에서 다시 들어올 수 있습니다.</p><p class="pento-tip">${['avalon','secrethitler','onenightwerewolf'].includes(room.game)?'이 게임은 실시간 소셜 디덕션 게임입니다. 역할·투표·정책/야간 정보는 각 참가자에게 필요한 정보만 표시됩니다.':room.game==='quacks'?'돌팔이 약장수는 여러 플레이어가 같은 라운드에서 각자 행동하는 동시 진행형 게임입니다. 상태 변경은 자동 동기화됩니다.':'모두가 동시에 접속할 필요는 없습니다. 방 목록에서 ‘내 차례’를 확인하고 자기 차례에 들어오면 됩니다. 모두 접속해 빠르게 진행해도 같은 방식으로 동작합니다.'} 방장은 대기실에서 참가자를 강퇴할 수 있습니다.</p></section>`);
    document.querySelector('#backMulti').onclick=()=>location.hash='#/multi';
    document.querySelector('#startRoom')?.addEventListener('click',async()=>{try{await callRpc('start_boardmate_room',{p_token:memberToken(),p_room_id:roomId});location.href=`./${gameInfo(room.game).page||(`online-${room.game}.html`)}?room=${encodeURIComponent(room.id)}`;}catch(e){toast(e.message);}});
