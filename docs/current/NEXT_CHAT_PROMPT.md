...
BoardMate Arena를 계속 개발한다. 현재 기준본은 **v11.4.79**이다.
...
- 알림: 실행 중 브라우저/PWA에서 best-effort 알림. 내 차례에는 브라우저 탭 제목 `🔔 내 차례 ·`. 숨김 탭 polling은 멈추고 `visibilitychange`, `focus`, `pageshow`, `resume`, `online` 복귀 시 즉시 재확인한다. 겹친 알림 평가와 목록 요청은 직렬화한다. 완전히 종료된 상태의 서버 Web Push는 아직 미구현.
- 캘리코: 루미 오른쪽 위 판정/재검사/게임 취소 유지.
- 에친스톤: 1인플 메뉴와 저장/이어하기 유지.
- 앱 아이콘: 사용자 제공 BoardMate 로고, 투명 배경.
- v11.4.79 신규 Supabase SQL/RPC 없음.

다음 우선순위 후보는 **행성 X 실사용 2인 이상 E2E 회귀**와 **전체 게임 자동 E2E 회귀 테스트**다. 완전 종료 Web Push는 현재 요구 범위 밖이다.
