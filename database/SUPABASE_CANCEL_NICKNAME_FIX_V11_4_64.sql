...
-- Fix: cancel RPC must read nickname from boardmate_profiles, not boardmate_room_members.
-- Also preserves the existing cancel-supported game list and adds Planet X / Eldorado / Greek Fantasy Realms.
...
begin
  ...
  ) then raise exception '이 방의 참가자가 아닙니다.'; end if;
  ...
  -- nickname belongs to boardmate_profiles, not boardmate_room_members.
  select coalesce(
    array_agg(coalesce(nullif(p.nickname,''),'플레이어') order by m.seat)
      filter(where coalesce(v.vote,false)=true),
    '{}'::text[]
  ) into voters
  from public.boardmate_room_members m
  left join public.boardmate_profiles p on p.user_id=m.user_id
  left join public.boardmate_game_cancel_votes v
    on v.room_id=m.room_id and v.user_id=m.user_id
  where m.room_id=p_room_id;

  return jsonb_build_object(
    'total',total_n,'yes',yes_n,'members',total_n,'votes',yes_n,'mine',coalesce(mine,false),
...
begin
  ...
  return jsonb_build_object(
    'total',total_n,'yes',yes_n,'members',total_n,'votes',yes_n,
    'mine',coalesce(p_vote,false),'voters',coalesce(voters,'{}'::text[]),
    'cancelled',cancelled,
