-- BoardMate Arcade v11.4.23 — Plakoro PvP V4
-- Run AFTER the existing BoardMate catalog / Plakoro V3 migration.
--
-- Based on the uploaded online-plakoro-multiplayer-setup-v3 HTML.
-- Updates:
--   * 6 Pokemon, each with 7 available moves / choose 4
--   * Energy dice: A1,A2,B1,B2 + C1,C2 where each C face contains 2 energy types
--   * Supports current energy types used by the uploaded source (including fighting/fight, wind/flying)
--   * Keeps BoardMate 2-player room flow and host-authoritative actions
--   * Keeps old saved room/setup rows readable; this migration does not delete state
