-- BoardMate Arena Social Deduction V1
-- Adds playable online Avalon, Secret Hitler (base game only), and One Night Ultimate Werewolf (base roles selected by BoardMate).
-- Prerequisite: current BoardMate schema (v11.4.x) with boardmate_profiles, boardmate_rooms,
-- boardmate_room_members, boardmate_session_user(), boardmate_get_room(), start_boardmate_room(), etc.
-- IMPORTANT: private game state is stored in boardmate_social_games.secrets and is NEVER granted to anon/authenticated.

begin;

-- -----------------------------------------------------------------------------
-- 1) Register games in existing BoardMate room constraints/helpers
-- -----------------------------------------------------------------------------
...
begin
  ...
  if p_game not in ('maskmen','acquire','calico','cascadia','pocketnova','thegame','kraken','fantasyrealms','powergrid','avalon','secrethitler','onenightwerewolf') then
  ...
  mode:=case when p_game in ('avalon','secrethitler','onenightwerewolf') then 'realtime' else 'turn' end;
...
-- Turn helper: realtime social-deduction rooms intentionally have no single current turn.
