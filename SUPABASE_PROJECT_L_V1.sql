-- BoardMate Arena v11.4.73 ALPHA
-- Project L multiplayer catalog / turn tracking / unanimous cancellation support.
-- Run ONCE in Supabase SQL Editor after uploading the v11.4.73 web files.
...
alter table public.boardmate_rooms add constraint boardmate_rooms_game_check
  check (game in (
    ...
    'panam','projectl','airlandsea','planetx'
  ));
...
returns integer language sql immutable as $$
  select case
    ...
    when p_game in ('thegame','projectl') then 5
...
returns integer language sql immutable as $$
  select case
    when p_game in ('calico','cascadia','pocketnova','thegame','plakoro','quacks','mandom','samurai','eldorado','panam','projectl','airlandsea','planetx') then 2
...
-- 4) Turn seat: Project L stores the active player in state.current ------------
...
begin
  ...
  if p_game in ('cascadia','thegame','kraken','fantasyrealms','fantasyrealmsgreek','plakoro','eldorado','panam','projectl','planetx') then return nullif(p_state->>'current','')::integer; end if;
