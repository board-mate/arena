...
function mountCancelControl(){
  ...
  const refresh=async()=>{if(busy)return;busy=true;try{draw(await cancelStatus());}catch(e){const msg=String(e.message||'');if(msg.includes('참가자'))wrap.style.display='none';else if(msg.includes('schema cache')||msg.includes('boardmate_get_cancel_status')){wrap.style.display='block';text.textContent='Supabase 취소 RPC가 아직 적용되지 않았습니다. v11.1의 SUPABASE_RPC_FIX_v11_1.sql을 SQL Editor에서 실행해 주세요.';voteBtn.disabled=true;}}finally{busy=false;}};
  open.onclick=()=>{panel.hidden=!panel.hidden;if(!panel.hidden)refresh();};
  wrap.querySelector('#bmCancelClose').onclick=()=>panel.hidden=true;
  voteBtn.onclick=async()=>{if(busy)return;if(!latest?.mine&&!confirm('게임 취소에 동의할까요? 참가자 전원이 동의하면 이 게임은 승패/ELO 반영 없이 종료됩니다.'))return;busy=true;voteBtn.disabled=true;try{draw(await setCancelVote(!latest?.mine));}catch(e){const msg=String(e.message||'');alert((msg.includes('schema cache')||msg.includes('boardmate_set_cancel_vote'))?'Supabase에 게임 취소 RPC가 아직 적용되지 않았습니다. v11.1의 SUPABASE_RPC_FIX_v11_1.sql을 SQL Editor에서 실행해 주세요.':(e.message||'취소 투표에 실패했습니다.'));}finally{busy=false;if(!String(text.textContent||'').includes('SUPABASE_RPC_FIX_v11_1.sql'))voteBtn.disabled=false;}};
