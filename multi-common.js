...
function mountCancelControl(){
  ...
  const refresh=async()=>{if(busy)return;busy=true;try{draw(await cancelStatus());}catch(e){if(String(e.message||'').includes('참가자'))wrap.style.display='none';}finally{busy=false;}};
  open.onclick=()=>{panel.hidden=!panel.hidden;if(!panel.hidden)refresh();};
  wrap.querySelector('#bmCancelClose').onclick=()=>panel.hidden=true;
  voteBtn.onclick=async()=>{if(busy)return;if(!latest?.mine&&!confirm('게임 취소에 동의할까요? 참가자 전원이 동의하면 이 게임은 승패/ELO 반영 없이 종료됩니다.'))return;busy=true;voteBtn.disabled=true;try{draw(await setCancelVote(!latest?.mine));}catch(e){alert(e.message||'취소 투표에 실패했습니다.');}finally{busy=false;voteBtn.disabled=false;}};
